...
\begin{document}
  ...
  Hlavní požadavek, který by měl produkt splňovat je \emph{otevřenost} (tj. bezplatná dostupnost) zdrojového kódu. Otevřenost výrazně usnadňuje kolaboraci (kdokoliv může navrhnout změnu) a zvyšuje důvěru v daný software (chování programu lze snadno ověřit nahlédnutím do kódu), což jsou žádoucí kvality softwaru zaměřeného na vzdělávání.
  ...
  Programovací jazyk je notace pro psaní programů, které ve své podstatě v slouží k provádění výpočtů. \emph{Syntaxe} jazyka popisuje strukturu~--~jakým způsobem za sebe uspořádat znaky, aby v rámci jazyka tvořily platná spojení, kdežto \emph{sémantika} jazyka popisuje význam těchto platných spojení~\cite{intro-to-programming-languages}.
  ...
  V projektu je používán \gls{sass}, který převádí \gls{css} s pokročilejší syntaxí a dodatečnou funkcionalitou do standardního \gls{css}. Díky vyšší míře abstrakce a dodatečným funkcím lze vzhled stránky upravovat o poznání pohodlněji.
  ...
  Na stránce zastává funkci skriptovacího jazyka pro automatizaci provozu (viz. kapitola~\ref{sec:Automatizace provozu webové stránky}). Oproti alternativám jako \gls{php} a Java je na stránce používán převážně z osobní preference.
  ...
  Decentralizovanost Gitu však existenci hlavní kopie nezamezuje~--~existuje řada služeb jako GitHub, GitLab či GitBucket, které centrální úschovny pro Gitem verzované projekty (jako tento dokument) hostují a poskytují tím při ztrátě či poškození lokální kopie dodatečnou ochranu.
  ...
  Fusion 360 je jediný v projektu používaný program, který nefunguje na operačním systému Linux. Za zmínku stojí otevřený\footnote{Zdrojový kód FreeCADu je dostupný na adrese \url{https://github.com/FreeCAD/FreeCAD}} multiplatformní program FreeCAD, se kterým však nemám dostatek zkušeností, proto nebyl v projektu používán.
  ...
  Následující kapitoly popisují, jakými způsoby byly části stránky (jako její hosting, logo, aj.) řešeny. Jelikož vývoj probíhal postupně, tak za nejdou chronologicky. % TODO: rephrase!
  ...
  Výsledky těchto testů jsou zaneseny do tabulky \ref{tab:Testování stránky na různých zařízeních}~--~správné fungování je značeno symbolem \faIcon{check}, špatné symbolem \faIcon{times} a nemožnost testování z důvodu nedostupnosti prohlížeče na dané platformně je značeno symbolem \faIcon{minus}.
  ...
  \begin{table}[H]
    ...
    \begin{tabular}{@{}rccccccccccccc@{}} \toprule
      ...
      && \faIcon{check} & \faIcon{check} & \faIcon{minus} & \faIcon{check} & \faIcon{check} & \faIcon{check} \\
        \bottomrule
    \end{tabular}
    \caption{Testování stránky na různých zařízeních}
    \label{tab:Testování stránky na různých zařízeních}
  \end{table}
  ...
  Vizualizace jsou implementovány v jazyce JavaScript s využitím knihovny p5.js (viz. kapitola~\ref{sec:p5.js}). Jsou uzpůsobeny pro ovládání myší či dotykem, aby byly funkční jak na osobních počítačích, tak na mobilech.
