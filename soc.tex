...
\setlength\heavyrulewidth{0.2ex}                % top and bottomrule thickness
...
\newcommand{\ra}[1]{\renewcommand{\arraystretch}{#1}} % for array row stretching
...
\usepackage[font=footnotesize,                  % 10pt captions
            ...
            belowskip=-12pt]{caption}           % reduced below skip
...
\usepackage{fontawesome}                        % for embedding icons
...
\begin{document}
  ...
  Příkladem \gls{html} je následující ukázka, která se skládá z hlavního nadpisu (\mintinline{html}{<h1>}~--~heading), následovaného paragrafem (\mintinline{html}{<p>}~--~paragraph), ve kterém se dále vyskytuje odkaz na webovou stránku (\mintinline{html}{<a>}~--~anchor):
  ...
  Příkladem \gls{css} je následující ukázka kódu, která upravuje vlastnosti nadpisu \mintinline{html}{<h1>}~--~mění barvu textu na zelenou, pozici na střed stránky a velikost na 18b:
  ...
  V rámci testování webové stránky jsou hlavní dvě kritéria: doba úplného načtení stránky při běžné rychlosti internetového připojení a její funkčnost na různých typech zařízení. % TODO: write more...
