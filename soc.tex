...
{\fontsize{14}{16.8} \selectfont Obor č. 18: Informatika}\\%
  ...
  Před začátkem je třeba určit požadavky, které by výsledný produkt měl splňovat. Ty budou fungovat jako kritéria, podle kterých lze varianty provedení hodnotit a použít tu, která je nejvhodnější.
  ...
  Příklady populárního otevřeného softwaru využívaného při stavbě tohoto projektu jsou např. Inkscape\cite{inkscape-source}, Atom\cite{atom-source}, Git\cite{git-source} nebo Jekyll\cite{jekyll-source}, které jsou podrobněji rozebírány v nadcházejících kapitolách (viz. kapitola \cite{sec:Použité nástroje}).
  ...
  Tato podmínka znemožňuje tvorbu aplikace, jelikož by se jak finanční\footnote{Licence na Appstore\cite{apple-store-membership} a Google Play\cite{android-store-membership} jsou výrazně dražší, než projektem používaný webhosting\cite{wedos-hosting}.}, tak časové nároky mnohonásobně zvýšily a komplexita projektu by přerostla nad rámec jednotlivce\cite{build-website-not-mobile-app}.


  \subsection{Statika obsahu}
  Produkt by neměl obsahovat dynamické části jako databáze či CMS (Content Management System). Hlavním důvodem tohoto požadavku je ochrana před potenciálními útoky, protože u statického obsahu fakticky není na co útočit - produkt si neukládá nic, co by mohl útočník zneužít. Tento požadavek by rovněž zjednodušil verzování, jelikož by témeř všechny části produktu byly text.
  ...
  Na českém trhu operuje veliké množství služeb, které zajišťují jak registraci domény, tak hostování samotné stránky. Každá má své výhody a nevýhody, proto je volba provozovatele vysoce individuální a záleží na řadě faktorů.

  Stránka je hostována a doména zprostředkována společností WEDOS Internet a jejich službou NoLimit, zejména kvůli mým pozitivním zkušenostem s tímto způsobem v rámci minulých projektů a faktem, že ceny a parametry jiných hostingů se od využívaného pro menší projekty příliš neliší.

  Jelikož je však Jekyll podporován GitHubem, který hostuje zdrojový kód stránky, další rozumná volba by byla hosting přes službu GitHub Pages\cite{github-pages}. Stránka by byla hostována zdarma, nebylo by potřeba obsah nahrávat přes FTP a generování stránky by probíhalo automaticky po provedení změny na straně GitHubu. Tento přístup by však znemožnil využívání scriptů pro stavbu stránky, proto není pro potřebné účely vhodný.
